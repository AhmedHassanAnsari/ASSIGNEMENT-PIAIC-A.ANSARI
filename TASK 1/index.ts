...
if(userDemand.GROCERY== 'fruit'){//here it check if user wanted to buy fruit,if it matches fruit then block of statement inside if are executed
    ...
    const optFruit=await inquirer.prompt([//used to take the name of fruit user selected to buy
        {
type:'list',
name:'selected',
...
choices: fruitChoices.map(choice => choice.name),
    ...
    const userResponse=await inquirer.prompt([//It will ask users if they want to generate bill of selected item
        {
            name:'response',
            type:'list',
...
const userResponse=await inquirer.prompt([// to ask if consumer want to generate bill for vegetable they opt
    {
        name:'response',
        type:'list',
message:'press yes if you want to generate a bill',
...
function tBill(choices: Array<{ name: string; price: number }>, selectedIndex: number): number {
    if (selectedIndex >= 0 && selectedIndex < choices.length) {
      const totalBill: number = choices[selectedIndex].price;//for accessing the price of the item selected by consumer
      return totalBill;
    } else {
...
//for discount
function discount(result:number){
let discountedResult:number;
